...
void INORDER_TRAVERSAL(RedBlackTree* Tree, Node* x) {
    if (x != Tree->NIL) {
        ...
        // 현재 노드
        printf("Key: %d, Color: %s, ", x->key, x->color == RED ? "RED" : "BLACK");

        // 왼쪽 자식
        if (x->left == Tree->NIL) {
            printf("Left Child: NIL, ");
        }
        else {
            printf("Left Child: %d, ", x->left->key);
        }

        // 오른쪽 자식
        if (x->right == Tree->NIL) {
            printf("Right Child: NIL\n");
        }
        else {
...
Node* RED_BLACK_TREE_SUCCESSOR(RedBlackTree* Tree, Node* x) {
    if (x->right != Tree->NIL) {
        return RED_BLACK_TREE_MINIMUM(Tree, x->right);
    }
    Node* y = x->parent;
...
int main(int argc, char* argv[]) {
    ...
    printf("You can use my program through these commands. The description of the button is as follows\n");
    printf("1. search : [s] \n2. minimum : [min]\n3. maximum : [max]\n4. next value : [n]\n5. previous value : [p]\n6. insertion : [i]\n7. delete : [d]\n8. terminate : [t]\n");
    ...
    while (1) {
        ...
        // 종료 (t)
            ...
            printf("Inorder traversal before exit:\n");
            INORDER_TRAVERSAL(Tree, Tree->root);
            printf("\n");
            break;
        }
